...
spmix.data<-argv[1] #pool catalog
...
# Create a matrix that will perform a mean-centering on the parametric covariance matrix  Then, mean-center the parametric ovariance matrix.
...
MC.parametric.covariance <- (MC.matrix) %*% last.params$admixed.covariance %*%  t(MC.matrix) #edit %*% is the operator to multiply matrix

# Finally, compare the sample covariance to the parametric covariance.  Ideally, there will be a very tight correspondence  between the data and the model.  
# If there is not, it may  be an indication either that the MCMC has not converged on the stationary distribution or that the process that generated 
# the data is only poorly approximated by SpaceMix's model.

# The sample and parametric covariances can be plotted against each other (if model fit is good they should  fall on the x=y red line)
...
plot(sample.covariance[index.matrix], 
    ...
    main="Model adequacy:\n matrix comparison")
    abline(0,1,col="red")                              
#We should definitely run longer MCMC and the output may be good
dev.off()
